...
//	private ImageUploadUtil imageup=new ImageUploadUtil();//图片上传
	...
	public BaseResponse save(HttpServletRequest request,
		...
		if(null != files && files.size() > 0){
			...
			if(null != dto.getDoctorTeamId()){//如果是teamId不为空，则为家庭医生zx咨询
				xtfile.setDescription("图文资讯上传");
				serviceType=2;
			}else{
				xtfile.setDescription("免费咨询上传");
				serviceType=8;
			}
		    urls=ImageUploadUtil.ImageUpload(files, quertionId, xtfile,serviceType,1);
		    if(urls.size() > 0){
		    	for (int i = 0; i < urls.size(); i++) {
		    		TwConsultAnswer anDto=new TwConsultAnswer();
			    	anDto.setCreateTime(date);
			    	anDto.setIsRead(0);
			    	anDto.setFormType(1);
			    	anDto.setFormId(Long.valueOf(userId));
			    	anDto.setConsultId(quertionId);
			    	anDto.setCountext(urls.get(i));
			    	anDto.setCountextType(1);
					twConsultAnswerServicel.saveAnswer(anDto);// 图文咨询问答保存
				}
		    }
		}
...
//			logger.warn("咨询次数减1:	结束  = " + (row == 0? "失败" : "成功"));
	...
	public BaseResponse saveAnswer(HttpServletRequest request,@RequestBody TwConsultAnswer dto) throws Exception{
		...
		if (null != dto.getFile() && dto.getFile().length() > 0) {
			Integer serviceType=null;
			if(quertDto.getAdvisoryType()==0){//快速提问
				serviceType=8;
			}else{
				serviceType=2;
			}
			...
			urls = ImageUploadUtil.ImageUpload(file, dto.getConsultId(), xtfile,serviceType,1);
		 ...
		 */
		if (dto.getReceptionFlag() == 1 && null != dto.getMsg()
				&& dto.getMsg().length() > 0) {
			String msg = dto.getMsg();
			if (null != urls && urls.size() > 0) {
				// 解析msg,放入图片url
				XtMessageCenter reportDto = JSONObject.parseObject(msg,
						XtMessageCenter.class);// 将json字符串转换成javabean
				String content = reportDto.getContent();
				Map<String, Object> contentMap = JSONObject.parseObject(
						content, HashMap.class);
				contentMap.put("img", urls);
				reportDto.setContent(JSONObject.toJSONString(contentMap));
				msg = reportDto.toJson();
			}
			if(!flag){//只有在主题未结束时才推送消息
				rep = nsqController.add(msg);
			}
		}
